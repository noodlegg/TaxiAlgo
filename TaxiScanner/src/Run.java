...
public class Run {
	...
	void init() {
		...
		graphPanel = new GraphPanel(graph, numOfTaxis);
	}
	...
	void doTestPhase() {
		...
		for (int i = 0; i < numOfTaxis; i++) {
			int startPos = 0;
			taxi[i].setCurrentPos(startPos);
			graphPanel.addTaxi(i, startPos);
		...
		for (int i = 0; i < totalPeriod; i++) {
			...
			for (int j = 1; j < input.length; j += 2) {
				...
				graphPanel.addPass(input[j], input[j + 1]);
			}
			//System.err.println();
